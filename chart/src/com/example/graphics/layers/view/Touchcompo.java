...
public class Touchcompo extends compo {
	...
	private void drawCross(Canvas canvas) {
		...
		System.out.println("index:"+index+"|touch:"+touchPostX+"|"+stickWidth);
		Entity entity = null;
		if(index>=0 && index + sd.getBeginIndex()<sd.getListEntity().size())
			entity = sd.getListEntity().get(index+sd.getBeginIndex());
		float closeY = 200;
		if(entity!=null)
			closeY = Utils.getCoordinate(entity.close, sd.getMaxValue(), sd.getMinValue(), super.getHeight()-48 );
		
		canvas.drawLine(48+index*stickWidth+stickWidth/2f, 0, 48+index*stickWidth+stickWidth/2f, this.getHeight(), paint);
